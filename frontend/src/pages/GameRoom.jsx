...
const GameRoom = () => {
  ...
  const handleGameEnd = async (gameMetrics) => {
    ...
    try {
      ...
      // Marcar juego como completado en esta sesión
      if (!completedGames.includes(selectedGame)) {
          setCompletedGames([...completedGames, selectedGame]);
      }

      // Si ganó un logro nuevo, lo agregamos para mostrarlo
      if (response.badge_awarded) {
          setChildAchievements(prev => [...prev, response.badge_awarded]);
      }

      setResult({ ...response, userScore: finalData.score }); 
      ...
      <div className="container position-relative py-2" style={{zIndex: 10}}>
        ...
        {result ? (
          ...
          <div className="rounded-5 shadow-lg overflow-hidden border border-4 border-white bg-black position-relative mx-auto mt-2" style={{ height: '75vh', maxWidth: '1000px' }}>
             {selectedGame === 'cpt' && <CPTGamePhaser onGameEnd={handleGameEnd} />}
             {selectedGame === 'go_no_go' && <GoNoGoGamePhaser onGameEnd={handleGameEnd} />}
             {selectedGame === 'tmt' && <TMTGamePhaser onGameEnd={handleGameEnd} />}
             {selectedGame === 'vigilance' && <VigilanceGamePhaser onGameEnd={handleGameEnd} />}
            ...
            {games.map((game) => {
              ...
              return (
                <div key={game.code} className="col-md-6 col-lg-3">
                  <div 
                      className={`card h-100 border-0 shadow-sm rounded-5 overflow-hidden position-relative transition-all 
                        ${isCompleted ? 'bg-light opacity-75 grayscale' : 'hover-lift cursor-pointer bg-white'}`}
                      onClick={() => !isCompleted && handlePlayGame(game.code)}
                      style={{ transform: isCompleted ? 'scale(0.98)' : 'scale(1)' }}
                  >
                    {isCompleted && (
