...
export default class CPTScene extends Phaser.Scene {
    ...
    create() {
        ...
        // --- CORRECCIÓN AQUÍ ---
        // Capa 1: Estrellas lejanas (Estáticas)
        this.starsFar = this.add.particles(0, 0, 'star', {
            x: { min: 0, max: 800 },
            y: { min: 0, max: 600 },
            // Quitamos 'quantity' de aquí para que no sea continuo
            lifespan: Number.MAX_VALUE, // Viven para siempre
            scale: { min: 0.2, max: 0.5 },
            alpha: { min: 0.3, max: 0.7 },
            emitting: false // IMPORTANTE: No emitir en bucle automáticamente
        });
        
        // Generamos las 100 estrellas UNA SOLA VEZ
        this.starsFar.explode(100); 
        // -----------------------
        ...
        this.starsNear = this.add.particles(0, 0, 'star', {
            ...
            y: { min: 0, max: 600 },
            lifespan: 2000, // Mueren a los 2 segundos (IMPORTANTE para no saturar)
        ...
        this.progressBar = this.add.rectangle(50, 580, 0, 8, 0x0ea5e9).setOrigin(0, 0.5); 
    ...
    nextTrial() {
        ...
        // Probabilidad del 30% de que sea objetivo (Estándar CPT)
    ...
    handleInput() {
        ...
        if (this.currentType === 'TARGET') {
            ...
            this.createExplosion(400, 300, 0xffd700);
            ...
            this.scoreText.setText('SCORE: ' + this.metrics.score);
            this.createExplosion(400, 300, 0xff0000);
    ...
    createExplosion(x, y, color) {
        const emitter = this.add.particles(x, y, 'star', {
            ...
            tint: color,
            blendMode: 'ADD',
            emitting: false // Aseguramos que no se quede emitiendo basura
        });
        emitter.explode(30); // Explotar una vez
        
        // Destruir el emisor completo después de la explosión para limpiar memoria
    ...
    endGame() {
        ...
        // Detener partículas
        if (this.starsNear) this.starsNear.destroy();
        if (this.starsFar) this.starsFar.destroy();
